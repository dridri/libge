...
int thread_start_point(void* p){
	...
	if(thread->flags & GE_THREAD_SHARED_GRAPHIC_CONTEXT || thread->flags & GE_THREAD_GRAPHIC_CONTEXT){
		//Make thread->graphic_ctx as current context
		LibGE_AndroidContext* ctx = (LibGE_AndroidContext*)libge_context->syscontext;
		const EGLint context_attribs[] = {
			EGL_CONTEXT_CLIENT_VERSION, 2,
			EGL_NONE
		};
		EGLContext context = eglCreateContext(ctx->display, ctx->config, ctx->context, context_attribs);
		gePrintDebug(0x100, "Thread context : 0x%08X (%d)\n", context, eglGetError());
		eglMakeCurrent(ctx->display, 0, 0, context);
		gePrintDebug(0x100, "eglMakeCurrent : %d\n", eglGetError());
		thread->graphic_ctx = (t_ptr)context;
	}
...
void geThreadStart(ge_Thread* thread, int args, void* argp){
	...
	pthread_create(&hThread, NULL, (void*(*)(void*))&thread_start_point, p);
...
void geThreadSuspend(ge_Thread* thread){
	thread->paused = true;
	pthread_kill((pthread_t)thread->handle, SIGUSR1);
}
...
static void SIGUSR1Handler(int errno){
	...
	gePrintDebug(0x102, "Thread '%s' in WaitState\n", thread->name);
	while(thread->paused){
		usleep(1000);
	}
}
...
void ge_PauseAllThreads(){
	int i = 0;
	for(i=0; i<32; i++){
		if(threads[i] && threads[i]->handle != (unsigned long)pthread_self()){
			gePrintDebug(0x100, "Pausing thread '%s'\n", threads[i]->name);
