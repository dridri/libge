...
void MP3_Init(ge_Music* mp3){
	...
	decoder->Stream = (u64)malloc(sizeof(struct mad_stream));
	decoder->Frame = (u64)malloc(sizeof(struct mad_frame));
	decoder->Synth = (u64)malloc(sizeof(struct mad_synth));
	decoder->Timer = (u64)malloc(sizeof(mad_timer_t));
	printf("MP3_Init 3\n");
...
static int thread_test(int args, void* argp){
	...
	while(1){
		...
		printf("\n");
		for(n=0; n<80; n++){
			if(n == decoder->samplesOut * 80 / (22050)){
				printf("#");
			}else{
				printf("-");
			}
...
int MP3_Thread(int args, void *argp){
	printf("MP3_Thread 1\n");
	ge_Music* mp3 = (ge_Music*)((u64*)argp)[0];
	ge_Mp3Decoder* decoder = (ge_Mp3Decoder*)mp3->decoder;
...
/*
	...
	unsigned long int ap[5] = { (t_ptr)mp3, (t_ptr)decoder, (t_ptr)&bc, (t_ptr)&w, 0 };
		...
		//	printf("Decoder: %d\nPlayer : %d\n\n", samples_read, cursorpos/2/2);
			...
			//	PrintFrameInfo(&Frame.header);
				if(FrameCount == 1){
					...
					bufsize = geAudioGetOutputBufferSize(mp3->handle);
				//	numSamples = bufsize / (sizeof(short) * 2);
					numSamples = 1152;
					FrameCount++;
					continue;
				}
			...
			//	mp3->cFrame = max(0, samples_read / 1152 - (64 + 32));
				//gePrintDebug(0x100, "Frame : %d\n", FrameCount);
				FrameCount++;

				mad_synth_frame(&Synth, &Frame);

				//pos = geAudioGetPlayerPosition(mp3->handle);
				pos = decoder->samplesOut * sizeof(short) * MAD_NCHANNELS(&Frame.header);
				cursorpos = pos;
				mp3->cFrame = samples_read;
				...
				for(i = 0; i < Synth.pcm.length; i++){
					...
					if(mp3->stopped){
						...
						memset(decoder->_buf, 0, bufsize*4);
						break;
					}
					...
					*/
					decoder->_buf2[i * 2 + 0] = decoder->_buf[decoder->samplesOut*2 + 0];
					decoder->_buf2[i * 2 + 1] = decoder->_buf[decoder->samplesOut*2 + 1];
			...
			geAudioOutputBlocking(mp3->handle, decoder->_buf, decoder->samplesOut);
//		}
	}
	return 0;
}
