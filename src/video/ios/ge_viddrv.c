...
//	int mipmap_detail = image->textureWidth / 256;
	...
	if(!image->id){
		...
		glTexImage2D(tex_mode, 0, GL_RGBA/*4*/, image->textureWidth, image->textureHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, image->data);
	...
	if(image->flags & GE_IMAGE_RECTANGLE){
		glTexParameterf(tex_mode, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		glTexParameterf(tex_mode, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	}else{
...
ge_Image* geAddTexture(ge_Image* tex){
	gePrintDebug(0x100, "geAddTexture(0x%08lX)...", (unsigned long)tex);
	//Compare
	ge_Image* ret = tex;
