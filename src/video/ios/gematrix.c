...
//	}
	...
	/*
	double cp[4] = { ge_clip_plane[0], ge_clip_plane[1], ge_clip_plane[2], ge_clip_plane[3] };
...
//	float m2[16];
	...
	//	geMatrix44Mult(m2, m, ge_matrices[GE_MATRIX_VIEW]);
	//	glMatrixMode(GL_MODELVIEW);
	//	glLoadMatrixf(m2);
		return;
	}else if(type == GE_MATRIX_VIEW){
	//	geMatrix44Mult(m2, m, ge_matrices[GE_MATRIX_MODEL]);
	//	glMatrixMode(GL_MODELVIEW);
	//	glLoadMatrixf(m2);
		return;
...
//	if(type==GE_MATRIX_MODELVIEW)gePrintDebug(0x100, "Updating matrix GE_MATRIX_MODELVIEW\n");
	if(mode==-1)return;
...
//	if(type<3 && !ge_matrix_update[type])return;
	...
	if(type == GE_MATRIX_MODEL)
		...
		i = loc_normal;
		glUniformMatrix3fv(i, 1, GL_FALSE, m);
		return;
	}
	else if(type == GE_MATRIX_TEXTURE0)
		i = loc_texture0;
...
//	if(type==GE_MATRIX_MODELVIEW)gePrintDebug(0x100, "Updating matrix GE_MATRIX_MODELVIEW (%d)\n", i);
	if(i<0)return;

//	gePrintDebug(0x100, "Sending matrix %d -> %d :", type, i);
//	PrintMatrix("", m, 4);
