...
#define CHUNK 16384
static void zlib_inflate(ge_Buffer* out, ge_Buffer* in);
static void zlib_deflate(ge_Buffer* out, ge_Buffer* in);
...
ge_Buffer* geCompress(void* in, int insz, int mode){
	...
	if(mode == GE_COMPRESSION_ZLIB){
		ge_Buffer b_in = { in, insz, 0 };
		zlib_deflate(out, &b_in);
	}
	return out;
}

static void zlib_inflate(ge_Buffer* b_out, ge_Buffer* b_in){
	int ret;
	unsigned have;
	z_stream strm;
